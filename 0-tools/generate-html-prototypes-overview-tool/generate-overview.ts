 ...
 */
const PROTOTYPES_DIR = '/workspace/docs/design/html-prototypes';
const OUTPUT_FILE = path.join(PROTOTYPES_DIR, '0-overview.md');
...
function buildComponentTree(element: Element): ComponentNode | null {
  ...
  // Find direct children with data-component
...
function groupChildren(
  children: ComponentNode[],
): Array<{ node: ComponentNode; count: number }> {
...
async function generateOverview() {
  ...
  for (const [i, file] of htmlFiles.entries()) {
    ...
    const allComponents = Array.from(
      dom.window.document.querySelectorAll('[data-component]'),
    );
